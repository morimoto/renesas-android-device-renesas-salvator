...
Stream::Stream(IStream *stream_io)
    : mStreamIO(stream_io) {
}
...
Result Stream::analyzeStatus(const char *funcName, int status,
    ...
    if (status != 0 && (ignoreErrors.empty()
                        || !element_in(-status, ignoreErrors))) {
...
Return<uint64_t> Stream::getBufferSize() {
    return mStreamIO->getBufferSize();
}

Return<uint32_t> Stream::getSampleRate() {
    return mStreamIO->getSampleRate();
}

Return<void> Stream::getSupportedSampleRates(getSupportedSampleRates_cb
        _hidl_cb) {
    String8 halListValue;
    Result result = getParam(AudioParameter::keyStreamSupportedSamplingRates,
                             &halListValue);
...
Return<Result> Stream::setSampleRate(uint32_t sampleRateHz) {
    return setParam(AudioParameter::keySamplingRate,
                    static_cast<int>(sampleRateHz));
}

Return<AudioChannelMask> Stream::getChannelMask() {
    return mStreamIO->getChannelMask();
}

Return<void> Stream::getSupportedChannelMasks(getSupportedChannelMasks_cb
        _hidl_cb) {
    String8 halListValue;
    Result result = getParam(AudioParameter::keyStreamSupportedChannels,
                             &halListValue);
...
Return<AudioFormat> Stream::getFormat() {
    return mStreamIO->getFormat();
}

Return<void> Stream::getSupportedFormats(getSupportedFormats_cb _hidl_cb) {
    String8 halListValue;
    Result result = getParam(AudioParameter::keyStreamSupportedFormats,
                             &halListValue);
    hidl_vec<AudioFormat> formats;
    Vector<audio_format_t> halFormats;
    if (result == Result::OK) {
        halFormats = formatsFromString(halListValue.string(),
                                       AudioParameter::valueListSeparator);
...
Return<void> Stream::getAudioProperties(getAudioProperties_cb _hidl_cb) {
    mStreamIO->getAudioProperties(_hidl_cb);
    return Void();
}

Return<Result> Stream::addEffect(uint64_t effectId) {
    return mStreamIO->addEffect(effectId);
}

Return<Result> Stream::removeEffect(uint64_t effectId) {
    return mStreamIO->removeEffect(effectId);
}

Return<Result> Stream::standby() {
    return mStreamIO->standby();
}

Return<AudioDevice> Stream::getDevice() {
    int device;
    Result retval = getParam(AudioParameter::keyRouting, &device);
    return retval == Result::OK ? static_cast<AudioDevice>(device) :
           AudioDevice::NONE;
}
...
Return<Result> Stream::setConnectedState(const DeviceAddress &address,
        bool connected) {
    return setParam(
               connected ? AudioParameter::keyStreamConnect :
               AudioParameter::keyStreamDisconnect,
               deviceAddressToHal(address).c_str());
}
...
Return<void> Stream::getParameters(const hidl_vec<hidl_string> &keys,
                                   getParameters_cb _hidl_cb) {
    getParametersImpl(keys, _hidl_cb);
    return Void();
}

Return<Result> Stream::setParameters(const hidl_vec<ParameterValue>
                                     &parameters) {
    return mStreamIO->setParameters(parameters);
}

Return<void> Stream::debugDump(const hidl_handle &fd) {
    if (fd.getNativeHandle() != nullptr && fd->numFds == 1) {
        mStreamIO->debugDump(fd);
    }
    return Void();
}
