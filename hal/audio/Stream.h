...
struct Stream : public IStream, public ParametersUtil {
    explicit Stream(IStream *stream_io);
...
struct StreamMmap : public RefBase {
    ...
    Return<void> createMmapBuffer(
        int32_t minSizeFrames, size_t frameSize,
...
template <typename T>
Return<void> StreamMmap<T>::createMmapBuffer(int32_t minSizeFrames,
        size_t frameSize,
        IStream::createMmapBuffer_cb _hidl_cb) {
...
template <typename T>
Return<void> StreamMmap<T>::getMmapPosition(IStream::getMmapPosition_cb
        _hidl_cb) {
    Result retval(Result::NOT_SUPPORTED);
